...
def login_user(request):
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            messages.error(request, 'Ошибка логина или пароля')
...
def logout_user(request):
    logout(request)
    messages.info(request, 'Пользователь вышел с сайта')
...
def create_skill(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            skill.owner = profile
            skill.save()
            messages.success(request, 'Умение добавлено успешно!')
            return redirect('account')
...
def update_skill(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, 'Умение обновлено успешно')
            return redirect('account')
...
def delete_skill(request, pk):
    ...
    if request.method == 'POST':
        skill.delete()
        messages.success(request, 'Умение удалено успешно!')
        return redirect('account')
